...
>
    <div x-data="turnoPortal()">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
            ...
            <div class="overflow-x-auto" x-show="canchas.length">
                <div class="min-w-[900px]">
                    <div class="grid border-b border-slate-100"
                        :style="'grid-template-columns: 220px repeat(' + horas.length + ', minmax(48px, 1fr))'">
                        <div class="px-4 py-2 text-[10px] font-bold uppercase text-slate-400 bg-slate-50 sticky left-0 z-10">Cancha</div>
                    ...
                    <template x-for="cancha in canchas" :key="cancha.id">
                        <div class="grid border-b border-slate-100"
                            ...
                            <div class="px-4 py-3 sticky left-0 z-10 bg-white border-r border-slate-100">
                                <p class="text-sm font-bold text-slate-800" x-text="cancha.nombre"></p>
                                <p class="text-[10px] text-slate-400" x-text="cancha.detalle"></p>
                            </div>
            ...
            <div class="px-6 py-10 text-center text-slate-500" x-show="!canchas.length" x-cloak>
                Esta sede aún no tiene canchas activas.
            </div>
        </div>

        <div class="mt-5 flex justify-end">
            <a href="{{ $urlDeporte }}" class="px-5 py-3 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-700 hover:bg-slate-50">
                Cambiar deporte
            </a>
        </div>
        ...
        function turnoPortal() {
            ...
            return {
                club, deporte: deporteParam, deporteId: deporteIdParam, fecha: fechaParam,
                ...
                get tusneActivo() {
                    ...
                    if (!c?.tusnes?.length) return null;
                    ...
                    let match = c.tusnes.find(t => (t.tipo_uso === uso || t.tipo_uso === 'todos') && (t.horario_turno === turno || t.horario_turno === 'todos'));
                    if (!match) match = c.tusnes.find(t => t.horario_turno === turno || t.horario_turno === 'todos');
                    return match || c.tusnes[0];
                },
                get precioHoraActual() {
                    if (this.tusneActivo?.precio_hora > 0) return Number(this.tusneActivo.precio_hora);
                ...
                precioDuracion(min) { return this.seleccion ? this.precioHoraActual * ((min||60)/60) : 0; },
                cambiarTipoUso(tipo) { if (this.seleccion) this.seleccion.tipoUso = tipo; },
                async cambiarDia(delta) {
                    const d = new Date(this.fecha + 'T12:00:00'); d.setDate(d.getDate() + delta);
                    const hoy = new Date(); hoy.setHours(0,0,0,0);
                    const max = new Date(hoy); max.setDate(max.getDate() + 6);
                    if (d < hoy || d > max) return;
                    this.fecha = d.toISOString().slice(0,10); this.cerrarPopup(); await this.cargarOcupacion();
                },
                async cargarOcupacion() {
                    ...
                    try {
                        ...
                        const res = await fetch(ocupacionUrl + '?' + params, { headers: { Accept: 'application/json' } });
                        const data = await res.json();
                        if (!data.ok) return;
                        const mapa = data.ocupados || {};
                        this.canchas = this.canchas.map(c => ({ ...c, ocupados: mapa[c.id] || mapa[String(c.id)] || [] }));
                    } finally { this.cargandoOcupacion = false; }
                },
                estaOcupado(c, h) { return (c.ocupados || []).includes(h); },
                ...
                claseCelda(c, h) {
                    if (this.estaOcupado(c,h)) return 'bg-slate-100 cursor-not-allowed';
                ...
                async continuar() {
                    ...
                    const params = new URLSearchParams({
                        ...
                        imagen: this.club.imagen||'', cancha: this.seleccion.cancha, cancha_id: String(this.seleccion.canchaId),
                        detalle: this.seleccion.detalle, fecha: this.fecha, hora: this.horaLabel(this.seleccion.hora),
                        duracion: String(this.seleccion.duracion), precio: String(this.precioDuracion(this.seleccion.duracion).toFixed(2)),
                        deporte: this.deporte, tipo_uso: this.seleccion.tipoUso,
                        tusne_id: tusne ? String(tusne.id) : '', codigo_tusne: tusne ? String(tusne.codigo) : '',
                        grupo_tusne: tusne ? String(tusne.grupo) : '23',
                    });
                    if (this.deporteId) params.set('deporte_id', String(this.deporteId));
                    else if (this.seleccion.deporteIds?.length) params.set('deporte_id', String(this.seleccion.deporteIds[0]));

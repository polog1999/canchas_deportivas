...
<head>
    ...
    <style>
        ...
        /* La imagen siempre ocupa TODO el slider */
...
<body class="bg-slate-100 text-slate-800 antialiased">
    ...
    <section class="relative bg-slate-900">
        <div id="heroCarousel" class="relative min-h-[420px] sm:min-h-[480px] lg:min-h-[520px] overflow-hidden">
            @forelse ($slides as $index => $slide)
                <div class="hero-slide {{ $index === 0 ? 'active-slide' : 'hidden-slide' }}"
                    data-slide="{{ $index }}">
                    <div class="absolute inset-0 bg-cover bg-center"
                        style="background-image: url('{{ $slide->urlImagen() }}')"></div>
            ...
            @empty
                <div class="hero-slide active-slide" data-slide="0">
                    <div class="absolute inset-0 bg-cover bg-center"
                        style="background-image: url('https://images.unsplash.com/photo-1574629810360-7efbbe195018?auto=format&fit=crop&w=1600&q=80')">
                    </div>
                    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-28 lg:py-32">
                        <h1
                            class="max-w-3xl text-3xl sm:text-4xl lg:text-5xl font-bold text-white leading-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.55)] break-words wrap-anywhere line-clamp-4">
        ...
        <div class="relative z-20 -mt-10 sm:-mt-12 px-4 sm:px-6 lg:px-8 pb-4">
            <div
                class="max-w-5xl mx-auto bg-[#1e3a5f] rounded-xl shadow-2xl p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-8">
            ...
            <div class="relative">
                ...
                <i
                    class="fa-solid fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-sky-700 text-[10px] pointer-events-none"></i>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8" id="gridSedes">
            @forelse ($sedes as $sede)
                ...
                @endphp
                <article
                    class="sede-card bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow"
                    data-sede-id="{{ $sede->id }}" data-deportes="{{ $deporteIds }}">
                    ...
                    <div class="p-5 sm:p-6">
                        ...
                        @if ($sede->hora_inicio && $sede->hora_fin)
                            <p class="text-xs text-slate-400 mt-1">
                                Horario: {{ substr((string) $sede->hora_inicio, 0, 5) }} –
                                {{ substr((string) $sede->hora_fin, 0, 5) }}
                            </p>
                        @endif
    ...
    <script>
        (function() {
            ...
            // =====================================================
            // CAMBIO DE SLIDE ARRASTRANDO CON EL MOUSE / TOUCH
            // =====================================================
            ...
            function filtrarSedes() {
                ...
                document.querySelectorAll('.sede-card').forEach((card) => {
                    ...
                    const matchSede = !sedeId ||
                        card.dataset.sedeId === sedeId;

                    const deps =
                        (card.dataset.deportes || '')
                        .split(',')
                        .filter(Boolean);

                    const matchDeporte = !deporteId ||
                        deps.includes(deporteId);
